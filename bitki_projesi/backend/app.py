...
# API anahtarı config.py dosyasından okunuyor (bu dosya .gitignore'da)
try:
...
# Yaprak kontrol modeli için eşik (skor bunun altındaysa "yaprak değil")
YAPRAK_ESIGI = 0.5
...
# ---------------------------------------------------------------------------
_model_cache = {}
_yaprak_model = None  # yaprak/değil kontrol modeli

AVAILABLE_MODELS = ['best_model', 'best_model_v2', 'best_model_v3', 'best_model_v4', 'best_model_v5', 'best_model_v6']

bitki_isimleri = {
    "Apple": "Elma", "Blueberry": "Yaban Mersini", "Cherry_(including_sour)": "Kiraz",
    "Corn_(maize)": "Mısır", "Grape": "Üzüm", "Orange": "Portakal", "Peach": "Şeftali",
    "Pepper,_bell": "Biber", "Potato": "Patates", "Raspberry": "Ahududu",
    "Soybean": "Soya Fasulyesi", "Squash": "Kabak", "Strawberry": "Çilek", "Tomato": "Domates",
}
...
def _get_model(model_name: str):
    ...
    model = keras.models.load_model(
        ...
        custom_objects={'Functional': keras.models.Model}, safe_mode=False
    )
    _model_cache[model_name] = model
    return model


def _get_yaprak_model():
    """Yaprak/değil kontrol modelini lazy-load eder."""
    global _yaprak_model
    if _yaprak_model is not None:
        return _yaprak_model
    yol = os.path.join(MODEL_DIR, 'yaprak_kontrol.keras')
    if not os.path.exists(yol):
        return None
    _yaprak_model = keras.models.load_model(yol, compile=False, safe_mode=False)
    return _yaprak_model


def _yaprak_mi(img_array):
    """Görüntünün yaprak olma olasılığını döndürür (0-1). Model yoksa None döner."""
    model = _get_yaprak_model()
    if model is None:
        return None
    skor = float(model.predict(img_array, verbose=0)[0][0])
    return skor


def _bos_bilgi(disease_tr):
    return {
        "hastalik_tr": disease_tr, "belirtiler": "", "organik_tedavi": "",
...
def get_classes():
    try:
        ...
        for idx in sorted(class_names.keys()):
            ...
            classes.append({
                'index': idx, 'class_name': cn,
                'class_name_tr': _translate_class_name_short(cn),
                'plant': plant_tr, 'plant_en': plant_en, 'disease': disease_tr,
                'disease_en': disease_en, 'is_healthy': is_healthy,
            })
...
def get_models():
    ...
    for name in AVAILABLE_MODELS:
        ...
        models.append({'name': name, 'exists': os.path.exists(path)})
...
def predict():
    try:
        ...
        # --- YAPRAK KONTROLÜ (yeni) ---
        # Hastalık tahmininden önce: bu görüntü gerçekten bir yaprak mı?
        yaprak_skoru = _yaprak_mi(img_array)
        if yaprak_skoru is not None and yaprak_skoru < YAPRAK_ESIGI:
            return jsonify({
                'is_leaf': False,
                'leaf_score': round(yaprak_skoru * 100, 2),
                'error': 'Bu bir yaprak fotoğrafı değil gibi görünüyor. Lütfen net bir yaprak fotoğrafı yükleyin.',
            }), 200
        ...
        return jsonify({
            'is_leaf': True,
            'leaf_score': round(yaprak_skoru * 100, 2) if yaprak_skoru is not None else None,
            'plant': plant_tr, 'plant_en': plant_en, 'disease': disease_tr,
            'disease_en': disease_en, 'is_healthy': is_healthy,
...
def get_metrics():
    ...
    if not os.path.exists(cache_path):
        return jsonify({'status': 'not_ready', 'message': 'Metrikler henüz hesaplanmadı.'})
    try:
...
def get_model_metrics(model_name):
    ...
    try:
        ...
        if model_name not in metrics.get('models', {}):
            ...
            return jsonify({'error': f'Model bulunamadı: {model_name}. Mevcut: {available}'}), 404
